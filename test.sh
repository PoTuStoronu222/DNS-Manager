...
mullvad_extended|security|ads+tracking+malware|Mullvad Extended|https://extended.dns.mullvad.net/dns-query|global|verified-published-current
...
mullvad_all|family|ads+tracking+malware+adult+gambling|Mullvad All|https://all.dns.mullvad.net/dns-query|global|verified-published-current
...
# Менеджер полностью управляет системным NTP-профилем:
# старые адреса удаляются, затем записываются только серверы выбранного профиля.
...
ok_msg "NTP: профиль '$NTP_PRESET' применён. Старые адреса времени удалены."
...
OWNERSHIP="$STATE_DIR/ownership.conf"
TEST_RESULTS="$STATE_DIR/dns-test-results.conf"
HDP_UPDATE_BEFORE="$CFG_DIR/https-dns-proxy-dnsmasq-update.before"
TMP_DIR="$(mktemp -d /tmp/dnsmgr.XXXXXX 2>/dev/null || { d="/tmp/dnsmgr.$$"; mkdir -p "$d"; printf "%s" "$d"; })"
TX_ID="$(date +%Y%m%d-%H%M%S)-$$"
TX_DIR="$STATE_DIR/tx-$TX_ID"
TX_ACTIVE=0
...
[ -f "$CONFIG_FILE" ] && . "$CONFIG_FILE" 2>/dev/null
_selfheal_migrate=0
[ -f "$CONFIG_FILE" ] && grep -q '^DNS_EMERGENCY_FALLBACK=' "$CONFIG_FILE" 2>/dev/null || _selfheal_migrate=1
: "${SLOT_1:=}"; : "${SLOT_2:=}"; : "${SLOT_3:=}"; : "${SLOT_4:=}"; : "${SLOT_5:=}"; : "${SLOT_6:=}"
: "${SLOT_RU:=}"; : "${SLOT_RU_2:=}"
: "${SLOT_1_CAT:=}"; : "${SLOT_2_CAT:=}"; : "${SLOT_3_CAT:=}"; : "${SLOT_4_CAT:=}"; : "${SLOT_5_CAT:=}"; : "${SLOT_6_CAT:=}"
...
: "${NTP_IP_FALLBACK:=1}"; : "${SYSCTL_TUNING:=0}"; : "${GO_OPTIMIZE:=0}"; : "${DNSMASQ_PERF:=0}"; : "${NTP_CLIENTS:=0}"; : "${CLIENT_FIXES:=0}"; : "${SYSCTL_EXTENDED:=0}"; : "${TAILSCALE_HOTPLUG:=0}"; : "${CRON_CLEANUP:=0}"
: "${BALANCER_ENABLED:=1}"; : "${NTP_PRESET:=cf_ip}"; : "${DNS_PROFILE:=hybrid}"
: "${DNS_EMERGENCY_FALLBACK:=1}"
: "${WATCHDOG_ENABLED:=1}"; : "${WATCHDOG_INTERVAL:=5}"
TLD_SPLIT="$TLD_RU_ENABLED"
if [ "$_had_dns_profile" = 0 ] && [ -z "$DNS_PROFILE" ]; then
DNS_PROFILE="hybrid"
fi
_legacy_migrated=0
for _legacy_slot in 1 2 3 4 5 6 RU RU_2; do
    eval "_legacy_cat=\${SLOT_${_legacy_slot}_CAT:-}"
    if [ "$_legacy_cat" = "social" ]; then
        case "$_legacy_slot" in
            RU|RU_2) eval "SLOT_${_legacy_slot}_CAT=regional" ;;
            *) eval "SLOT_${_legacy_slot}_CAT=bypass" ;;
        esac
        _legacy_migrated=1
    fi
done
...
if [ "${_selfheal_migrate:-0}" = 1 ] && [ "$DNS_PROFILE" = hybrid ]; then
    WATCHDOG_ENABLED=1
    WATCHDOG_INTERVAL=5
    DNS_EMERGENCY_FALLBACK=1
fi
[ "${_legacy_migrated:-0}" = 1 ] && save_config 2>/dev/null || true
[ "${_selfheal_migrate:-0}" = 1 ] && save_config 2>/dev/null || true
}
save_config() {
umask 077
cat > "$CONFIG_FILE" <<EOF_CFG
...
WATCHDOG_ENABLED="$WATCHDOG_ENABLED"
WATCHDOG_INTERVAL="$WATCHDOG_INTERVAL"
DNS_EMERGENCY_FALLBACK="$DNS_EMERGENCY_FALLBACK"
EOF_CFG
}
# ==========================================
# ОБНАРУЖЕНИЕ СИСТЕМЫ И ЗАВИСИМОСТЕЙ
...
# DoH — ПРИМЕНЕНИЕ КОНФИГУРАЦИИ
# ==========================================
isolate_https_dns_proxy() {
    # Manager owns dnsmasq routing; prevent https-dns-proxy init logic from rewriting it on restart.
    if [ ! -f "$HDP_UPDATE_BEFORE" ]; then
        uci -q get https-dns-proxy.config.dnsmasq_config_update > "$HDP_UPDATE_BEFORE" 2>/dev/null || : > "$HDP_UPDATE_BEFORE"
    fi
    uci -q set https-dns-proxy.config.dnsmasq_config_update='-' 2>/dev/null || return 1
    uci -q commit https-dns-proxy 2>/dev/null || return 1
    return 0
}

restore_https_dns_proxy_update_mode() {
    [ -f "$HDP_UPDATE_BEFORE" ] || return 0
    _old_update="$(cat "$HDP_UPDATE_BEFORE" 2>/dev/null)"
    if [ -n "$_old_update" ]; then
        uci -q set https-dns-proxy.config.dnsmasq_config_update="$_old_update" || return 1
    else
        uci -q delete https-dns-proxy.config.dnsmasq_config_update || true
    fi
    uci -q commit https-dns-proxy || return 1
    rm -f "$HDP_UPDATE_BEFORE" 2>/dev/null
    return 0
}

watchdog_local_query() {
    _port="$1"
    [ -n "$_port" ] || return 1
    if command -v dig >/dev/null 2>&1; then
        dig +time=2 +tries=1 @127.0.0.1 -p "$_port" example.com A >/dev/null 2>&1
    elif command -v nslookup >/dev/null 2>&1; then
        nslookup example.com 127.0.0.1 >/dev/null 2>&1
    else
        netstat -an 2>/dev/null | grep -qE ":${_port}([[:space:]]|$)"
    fi
}

watchdog_restart_service() {
    isolate_https_dns_proxy >/dev/null 2>&1 || true
...
emergency_dns_enable() {
    [ "${DNS_EMERGENCY_FALLBACK:-1}" = 1 ] || return 0
    _sec="$(get_dnsmasq_section)"
    [ -n "$_sec" ] || return 1
    _cur="$(uci -q get "dhcp.$_sec.noresolv" 2>/dev/null)"
    if [ "$_cur" = 1 ]; then
        uci set "dhcp.$_sec.noresolv=0" || return 1
        uci commit dhcp || return 1
        /etc/init.d/dnsmasq restart >/dev/null 2>&1 || true
    fi
    touch "$STATE_DIR/dns-emergency.active" 2>/dev/null
    log_msg "EMERGENCY DNS: все локальные DoH недоступны; временно разрешён DNS от WAN (noresolv=0)."
    return 0
}

emergency_dns_disable() {
    [ -f "$STATE_DIR/dns-emergency.active" ] || return 0
    _sec="$(get_dnsmasq_section)"
    [ -n "$_sec" ] || return 1
    uci set "dhcp.$_sec.noresolv=1" || return 1
    uci commit dhcp || return 1
    /etc/init.d/dnsmasq restart >/dev/null 2>&1 || true
    rm -f "$STATE_DIR/dns-emergency.active" 2>/dev/null
    log_msg "EMERGENCY DNS: локальные DoH восстановлены; DNS от WAN отключён, noresolv=1."
    return 0
}

clear_all_doh_for_apply() {
    printf "${C_PINK}↻ Удаляются только DoH-секции DNS Manager; чужие секции сохраняются.${C_NC}\n"
    _i=0
    _removed=0
    while uci -q get "https-dns-proxy.@https-dns-proxy[$_i]" >/dev/null 2>&1; do
        ...
        if [ "$_m" = 1 ]; then
            _u="$(uci -q get "https-dns-proxy.@https-dns-proxy[$_i].resolver_url" 2>/dev/null)"
            [ -n "$_u" ] && printf "  ${C_PINK}↻ Удаляется собственный DoH: %s${C_NC}\n" "$_u"
            uci -q delete "https-dns-proxy.@https-dns-proxy[$_i]" || return 1
            _removed=$((_removed+1))
            continue
        fi
    ...
    uci commit https-dns-proxy || return 1
    disc_dns
    printf "${C_GREEN}✓ Собственных DoH удалено: %s. Чужие DoH не изменены.${C_NC}\n" "$_removed"
    return 0
}
record_own() { printf '%s|%s|%s|%s\n' "$1" "$2" "$3" "$4" >> "$OWNERSHIP"; }
ensure_doh_slot() {
...
[ "$CRON_CLEANUP" = 1 ] && printf "  ${C_GREEN}✓${C_NC} Чистка старых cron-задач DNS Manager\n" || printf "  ${C_YELLOW}—${C_NC} Cron не изменяется\n"
[ "$WATCHDOG_ENABLED" = 1 ] && printf "  ${C_GREEN}✓${C_NC} Автопроверка DoH: каждые %s мин\n" "$WATCHDOG_INTERVAL" || printf "  ${C_YELLOW}—${C_NC} Автопроверка DoH не изменяется\n"
[ "$DNS_EMERGENCY_FALLBACK" = 1 ] && printf "  ${C_GREEN}✓${C_NC} Аварийный DNS при полном отказе DoH (только на время аварии)\n" || printf "  ${C_YELLOW}—${C_NC} Аварийный DNS fallback выключен\n"
fi
printf "\n${C_WHITE}Текущее состояние до применения:${C_NC}\n"
printf "  dnsmasq: %b\n" "$(state_word "$DNSMASQ_RUN")"
printf "  DoH: %s (настройка %s / внешние %s / без определения %s)\n" "$DOH_TOTAL" "$DOH_OURS" "$DOH_FOREIGN" "$DOH_UNKNOWN"
if [ "$DOH_TOTAL" -gt 0 ]; then
printf "  ${C_PINK}↻ После подтверждения будут заменены только DoH DNS Manager; чужие DoH сохранятся.${C_NC}\n"
printf "  ${C_YELLOW}   До изменения создаётся снимок для автоматического отката при ошибке.${C_NC}\n"
fi
printf "\n${C_YELLOW}Только после подтверждения будет создан снимок и внесены изменения.${C_NC}\n"
printf "${C_YELLOW}ℹ Если желаемый порт занят чужим сервисом, слот уедет на ближайший свободный — итог будет показан после применения.${C_NC}\n"
...
tx_snapshot_start || { err_msg "Не удалось создать снимок транзакции. Изменения не выполняются."; return 1; }
log_tx "PLAN" "all" "APPLY" "START" "version=$VERSION"
isolate_https_dns_proxy || { err_msg "Не удалось отключить автоматическое изменение dnsmasq со стороны https-dns-proxy."; tx_restore_on_failure; return 1; }
...
uci commit https-dns-proxy 2>/dev/null
sec="$(get_dnsmasq_section)"
uci -q set "dhcp.$sec.noresolv=0" || true
...
restore_https_dns_proxy_update_mode >/dev/null 2>&1 || true
/etc/init.d/https-dns-proxy restart 2>/dev/null
/etc/init.d/dnsmasq restart 2>/dev/null
printf "${C_GREEN}✓ Свои основные DoH/dnsmasq/firewall/sysctl/NTP изменения обработаны.${C_NC}\n"
printf "${C_YELLOW}! Изменённые вручную файлы/объекты не перезаписывались.${C_NC}\n"
...
printf "  Авто-перезапуск Tailscale:  %s\n" "$(module_state_word ts_hotplug "$TAILSCALE_HOTPLUG")"
printf "  Фиксы телеметрии/связи:     %s\n" "$(module_state_word client_fixes "$CLIENT_FIXES")"
printf "  Аварийный WAN DNS fallback: %s\n" "$(config_state_word "$DNS_EMERGENCY_FALLBACK")"
printf "${C_GREEN}✓ Discovery завершён. Изменений в конфигурацию не внесено.${C_NC}\n"
pause
}
# ==========================================
...
if auto_fill_slots "$goal"; then
DNS_PROFILE="custom"
# Ready categories are not Hybrid: keep their selected DNS set instead of letting Hybrid reassert itself.
save_config
CORE_ONLY=1
apply_settings
CORE_ONLY=0
...
menu_note "✓ allservers=1"
menu_note "✓ уникальные порты"
menu_note "✓ чужие DoH сохраняются и не удаляются"
menu_note "✓ проверка после применения + автоматический откат"
printf "${C_YELLOW}⚠${C_NC} DNS не заменяет Zapret/VPN при блокировках по IP, SNI, DPI и HTTP.\n"
test_dns_catalog
[ -s "$TEST_RESULTS" ] || return
...
watchdog_check_slot() {
    ...
    _url="$(normalize_url "$(dns_url "$_id")")"
    [ -n "$_url" ] || return 1
    # First check the actual local proxy instance. This catches a hung https-dns-proxy process.
    _inv="$(find_own_doh_by_url "$_url")"
    _port="$(printf '%s' "$_inv" | cut -d'|' -f2)"
    [ -n "$_port" ] || return 1
    watchdog_local_query "$_port" || return 1
    # Then confirm the upstream resolver itself is still usable.
    verify_doh_endpoint "$_url" "$(dns_name "$_id")" >/dev/null 2>&1 || return 1
    return 0
}
...
run_watchdog() {
    ...
    run_discovery
    load_config
    isolate_https_dns_proxy >/dev/null 2>&1 || log_msg "Watchdog: не удалось зафиксировать режим dnsmasq_config_update."
    run_discovery
    ...
    # If the local DNS path is already dead, restart only the proxy first.
    # A restart is cheaper and safer than rebuilding the whole configuration.
    _local_dns_ok=0
    for _probe in 1 2 3; do
        if command -v dig >/dev/null 2>&1 && dig +time=2 +tries=1 @127.0.0.1 example.com A >/dev/null 2>&1; then
            _local_dns_ok=1
            break
        fi
        sleep 1
    done
    if [ "$_local_dns_ok" = 0 ]; then
        log_msg "Watchdog: локальный DNS не отвечает. Перезапускаю только https-dns-proxy перед заменой слотов."
        watchdog_restart_service || true
        _local_dns_ok=0
        for _probe in 1 2 3; do
            if command -v dig >/dev/null 2>&1 && dig +time=2 +tries=1 @127.0.0.1 example.com A >/dev/null 2>&1; then
                _local_dns_ok=1
        ...
        [ "$_local_dns_ok" = 1 ] || emergency_dns_enable >/dev/null 2>&1 || true
    else
        emergency_dns_disable >/dev/null 2>&1 || true
    fi
...
if watchdog_check_slot "$_id"; then
    ...
    continue
        ...
        for _attempt in 1 2 3; do
            ...
            save_config
            log_msg "Замена DoH слота $_slot: попытка $_attempt не прошла, откат выполнен."
            emergency_dns_enable >/dev/null 2>&1 || true
            [ "$_attempt" -lt 3 ] && sleep 2
        done
...
apply_watchdog() {
    ...
    _interval="${WATCHDOG_INTERVAL:-5}"
    case "$_interval" in
        ''|*[!0-9]*) _interval=5 ;;
    esac
    [ "$_interval" -ge 1 ] 2>/dev/null || _interval=5
    [ "$_interval" -le 59 ] 2>/dev/null || _interval=59
    WATCHDOG_INTERVAL="$_interval"
...
printf "  ${C_YELLOW}${C_BOLD}DoH обнаружено${C_NC}     ${C_YELLOW}${C_BOLD}%s${C_NC}\n" "$DOH_TOTAL"
printf "  ${C_YELLOW}${C_BOLD}Watchdog${C_NC}            %b\n" "$(module_state_word watchdog "$WATCHDOG_ENABLED")"
[ -f "$STATE_DIR/dns-emergency.active" ] && printf "  ${C_YELLOW}${C_BOLD}Аварийный DNS${C_NC}        ${C_YELLOW}WAN DNS временно разрешён${C_NC}\n" || true
[ "$FORCE_DNS" = 1 ] && printf "  ${C_YELLOW}${C_BOLD}⚠ force_dns${C_NC} ${C_CYAN}стороннего DoH включён${C_NC}\n"
...
run_discovery
# Protect Manager-owned routing from https-dns-proxy init-time dnsmasq rewrites.
if [ "$DOH_OURS" -gt 0 ]; then
    isolate_https_dns_proxy >/dev/null 2>&1 || log_msg "SELFHEAL: не удалось зафиксировать dnsmasq_config_update при старте."
fi
if [ "$DNS_PROFILE" = hybrid ] && [ "$DOH_OURS" -gt 0 ]; then
    _sec="$(get_dnsmasq_section)"
    _nr="$(uci -q get "dhcp.$_sec.noresolv" 2>/dev/null)"
    if [ "$_nr" = 0 ]; then
        uci -q set "dhcp.$_sec.noresolv=1" && uci -q commit dhcp && log_msg "SELFHEAL: после старта Hybrid восстановлен noresolv=1."
        /etc/init.d/dnsmasq restart >/dev/null 2>&1 || true
    fi
fi
# Watchdog is enabled by default in 1.13; make sure its cron entry actually exists.
[ "${WATCHDOG_ENABLED:-0}" = 1 ] && apply_watchdog >/dev/null 2>&1 || true
